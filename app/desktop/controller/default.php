...
//      $this->pagedata['certificate_url'] = "http://key-service.shopex.cn/?sess_id=".urlencode($this->pagedata['session_id'])."&certi_id=".urlencode($this->Certi)."&version=ecstore&confirmkey=".urlencode($confirmkey)."&_key_=do";
        ...
        $this->pagedata['certificate_url'] = config::get('link.shopex_license_active_search'). "?product=".kernel::single('syssupport_auth')->getProduct() . '&code=' . urlencode(kernel::single('syssupport_auth_code')->encode(kernel::single('syssupport_auth_code')->getParams(), 3600*24*7));
    ...
    {
        echo kernel::single('base_license_active')->getGif();exit;
    }
