...
class desktop_mdl_menus extends dbeav_model{
   ...
   return $data;        
    ...
    function permissionId($get){
        ...
        $menu_count = count($aData);
        if($menu_count > 1){
            $noaddonkey = ''; // menu没有附加参数的key
            $addonkey = ''; //menu有附加参数的key
            foreach($aData as $k => $row){
                $addon = unserialize($row['addon']);
                if(!$addon['url_params']){
                    $noaddonkey = $k;
            ...
            if($addonkey){//有附加参数的则返回有附加参数的权限
              return $aData[$addonkey]['permission'];
            }else{//无附加参数的则返回无附加参数的权限
              return $aData[$noaddonkey]['permission'];
            }
        }elseif($menu_count == 1){
            if($row['disabled'] == 1){
                echo "链接不可用";
