...
class base_database_manager
{
	...
	public function connection($name = null)
    {
        ...
        $connection = !isset($this->connections[$name]) ? $this->makeConnection($name) : $this->connections[$name];

        // 当指定了type, 同时使用master slave模式的情况下, 设置master或者slave
        if ( !is_null($type) && $connection instanceof Doctrine\DBAL\Connections\MasterSlaveConnection)
        {
    ...
    public function getConfig($name = null)
    {
        ...
        // 如果获取的配置是最初的初始化配置，直接将host引向127.0.0.1
        // %CONNECTIONS.DEFAULT.HOST%
        // 理由是这个诡异的host在DNS不够强劲的情况下，会导致这里在找服务器的时候超时
        if($config['host'] == '%CONNECTIONS.DEFAULT.HOST%') $config['host'] = '127.0.0.1';
