...
class topdev_apis {
    ...
    public function getTopManageApiGroupList($v='v1')
    {
        $apis = $this->getApiList('topmanageapi', $v);

        $redis = redis::scene('topdev');
        $redisKey = 'topmanageapi'. md5(json_encode($apis));
        $list = $redis->get($redisKey);
        if( !$list )
        {
            foreach ($apis as $key => $value)
    ...
    public function getSystemParams($isAuth, $type , $v='v1')
    {
        ...
        if( $isAuth && in_array($type,['topapi', 'topmanageapi']))
        {
            $return[] = [
                'field' => 'accessToken',
                'type' => 'string',
    ...
    public function getApiList($type, $v='v1')
    {
        ...
        {
        case 'topapi':
            return config::get('topapi.routes.'.$v);
        case 'topmanageapi':
            return config::get('topmanageapi.routes.'.$v);
        default :
            return config::get('apis.routes');
        }
        return $apis;
    }
